...
export function TeamFeed({ team }: TeamFeedProps) {
    ...
    return (
        <div className="">
            ...
            <div className={`grid gap-x-2 md:gap-x-4 my-4 max-w-4xl ${selectedSeason !== 'all' ? 'grid-cols-[max-content_max-content_auto]' : 'grid-cols-[max-content_max-content_max-content_auto]'}`}>
                ...
                {filteredFeed.map((event, i) => {
                    ...
                    for (const link of event.links) {
                        if (link.match === `${team.emoji} ${team.location} ${team.name}`)
                            continue;
